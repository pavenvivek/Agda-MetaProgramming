...
\todo{I think this is a more accurate presentation of what's going on - what do you think?}In Agda, we define an inductive type using {\tt data} keyword. A definition of an inductive datatype declares its type and specifies its constructors.
...
&\hspace{0.3cm} c_1 : \Delta_1 \rightarrow D \, a_1 \ldots a_n \, j_{11} \ldots j_{1p}  \\
&\hspace{3.5cm} \vdots  \\
&\hspace{0.3cm} c_r : \Delta_n \rightarrow D \, a_1 \ldots a_n \, j_{n1} \ldots j_{nq}  \\
\end{align*}
\endgroup
\end{center}
\normalsize
where $j_{k1} \ldots j_{kp}$ are bound in the telescope $\Delta_k$.
...
\normalsize
  ...
  \begin{align*}
D_{\mathit{elim}} : & (a_1 : A_1) \to \ldots \to (a_n : A_n) \to\\
& \begin{array}{@{}l@{}l}(\mathit{mot} :\ & (i_1 : I_1) \to \ldots \to (i_m : I_m) \to\\  & D\ a_1 \ldots a_n\ i_1\ \ldots\ i_n \to\\ &\SET{})\to\end{array} \\
& (\mathit{f_1} : \Delta_1 \to  \Delta_1^\prime \to \mathit{mot}\ j_{11} \ldots j_{1p}\ (c_1\ \overline{\Delta_1})) \to \\
& (\mathit{f_r} : \Delta_r \to  \Delta_r^\prime \to \mathit{mot}\ j_{r1} \ldots j_{rp}\ (c_r\ \overline{\Delta_r})) \to \\
& (i_1 : I_1) \to \ldots \to (i_m : I_m) \to\\
& (\mathit{tgt} : D\ a_1 \ldots a_n\ i_1\ \ldots\ i_n) \to\\
& \mathit{mot}\ i_1\ \ldots\ i_n\ \mathit{tgt}
  \end{align*}

where $\overline{\Psi}$ represents the sequence of variables bound in the telescope $\Psi$, and $j_{ij}$ are bound in $\Delta_i$.
...
& \hspace{0.4cm} (c_1 : \{i_1:Q_1\} \rightarrow\ldots\rightarrow \{i_n:Q_n\} \rightarrow A{_1}^* \rightarrow C (g_1 ...)) \rightarrow \ldots \nonumber \\
...
Similar to the recursion principle, the type of $A{_i}^*$ in constructor $c_i$ depends on the type of $A_i$ in the  constructor $g_i$. If $A_i$ is a constant type $B$, the automation tool replaces $A{_i}^*$ with $B$. For inductive type $D$, the automation tool replaces $A{_i}^*$ with type $(d_1 : D) \rightarrow C \, d_1$. For function type $(B_1 \rightarrow\ldots\rightarrow B_n \rightarrow D)$, the automation tool replaces $A{_i}^*$ with $(y : B_1 \rightarrow\ldots\rightarrow B_n \rightarrow D) \rightarrow ((b_1 : B_1) \rightarrow\ldots\rightarrow (b_n : B_n) \rightarrow C (y \, b_1 \ldots b_n))$. The computation rules corresponding to the induction principle follows the same pattern as for the recursion rule.
