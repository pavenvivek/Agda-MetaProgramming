...
While more recent work arranges the computational mechanisms of the type theory such that univalence can be derived, as is done in cubical type theories~\citep\citep{cohen, Angiuli-2017}, we are concerned with modeling concepts from homotopy type theory in existing, mature implementations of type theory, so we follow \citet{HoTT-2013} in modeling paths using Martin-Löf's identity type.
...
Our metaprogram currently supports only one-dimensional paths,\todo{THIS IS A MAJOR LIMITATION THAT NEEDS TO BE IN THE INTRODUCTION!!!!!!!!!!!} but we are planning to improve the tool to support higher-dimensional paths in the future. 
...
The automation tool builds the type of \texttt{loop}'s method by traversing the AST of \texttt{loop}'s type,
...
The automation tool applies the eliminator of base type {\tt S*} to map {\tt base} to {\tt cbase} during the construction of the codomain of {\tt cloop}. The following declaration gives the type of {\tt indS}.
\begin{center}
\begin{BVerbatim}
indS : (circle : S) → 
  (C : S → Set) →
...
%We would also like to automate code generation for more members of the inductive type family such as the inductive-inductive type and the inductive-recursive type.
