...
\todo{The beginning of this section needs a general-purpose description of what's going on, that at least sketches the high-level process. Then, Vec can be used as an example. But an example alone is not sufficient. This para is mostly a sketch to be filled in.}
While inductive datatypes are defined by their constructors, it must also be possible to \emph{eliminate} them.
This section describes how to generate a non-dependent recursion principle for an inductive type; section~\ref{sec:sec3.2} generalizes this technique to fully-dependent induction principles.\todo{Give a schematic presentation of the recursor of an inductive type here, then instantiate it using Vec. The schematic presentation should match the Agda code to generate it.}
...
For {\tt Vec}, the recursion principle says that to define a mapping $f : Vec \rightarrow C$\todo{Two issues: why math mode instead of Agda? And what happened to the parameter and the index? Those things are important and shouldn't be swept under the rug!}, it suffices to define the action of \emph{f} on inputs {\tt []} and {\tt \_::\_}.
...
The function \emph{f} will map the constructors {\tt []} and {\tt \_::\_} to {\tt c1} and {\tt c2} respectively.\todo{Use present tense, not future tense, unless it really only is true in the future.}\todo{This seems a bit off, shouldn't it instantiate those two methods with the appropriate arguments, and also with the results of recursion on Vec-typed substructures?} To construct the type of the recursion rule for {\tt Vec}, we need to build the type of {\tt c1} and {\tt c2}. Since {\tt []} is not a function type, we can map it directly to {\tt c1 : C}. We can retrieve the static type information of {\tt \_::\_} using reflection primitives, and use that to construct the type of {\tt c2}. The constructor {\tt pi} of type {\tt Term} encodes the abstract syntax tree (AST) representation of {\tt \_::\_} (fig.~\ref{fig:ast-g}). We can retrieve and traverse the AST of {\tt \_::\_}, and add new type information into it to build a new type representing {\tt c2}.
...
During the traversal of abstract syntax tree of the type of {\tt \_::\_}, when we identify a function {\tt f1} with codomain {\tt Vec}, we add a new function {\tt f2}, with the same arguments as {\tt f1} and codomain {\tt C}, to the tree.\todo{Why? Without the schematic presentation, this is hard to follow. Here, it is the result of the recursion, which is what makes this into primitive recursion.} When the type {\tt Vec} occurs directly in a non-codomain position, we add the type {\tt C} next to it. For example, in figure~\ref{fig:ast-g}, a new function is built from the argument {\tt (xs : Vec A n)} by modifying it to {\tt (Vec A n) → C} (fig.~\ref{fig:ast-d}). Constant types require no modifications. Therefore, we copy {\tt (x : A)} into the new type without any changes. Finally, we change the codomain {Vec A (suc n)} of {\tt \_::\_} to \emph{C} resulting in an abstract syntax tree representation of {\tt c2}\todo{Wouldn't that be c2's type? Be careful when talking about a reference to a name, a reference to what the name refers to, and a reference to the type of what the name refers to. These things are all different, and it matters when writing about metaprogramming.} (fig.~\ref{fig:ast-d}). We repeat this process for each of the constructors.\todo{The nesting of pi and abs seems unfortunate - can we write a helper that does both at once?}
...
Constructors with arguments of function type with the inductive type being defined as the codomain are handled differently.\todo{Is that the case? Aren't the previous rules the nullary case of what you're going to describe?} For example, consider a constructor $g$ of the following type.\todo{Give the actual Agda definition of the version of the W type that you're using. Also, the constructor should be called \texttt{sup}, which is PML's name (``suprememum'')}
\begin{equation}
...
In Agda,\todo{This looks more like Dybjer's or Luo's presentations of datatypes, not Agda's...} we define an inductive type using the {\tt data} keyword\todo{Totally unnecessary to give the keyword - anyone who can read up to here either knows this or can work it out.}. A datatype declaration characterizes a datatype by declaring its type and specifying its constructors. Consider the following generic form for the definition of an inductive type \emph{W} \todo{Use a better metavariable} with constructors $g_1 \ldots g_n$\todo{Why not c for constructors?}.\todo{This more schematic presentation should come much earlier}
\begin{center}
\begingroup
...
The constructor $argPath$ takes the type of a path constructor as input. We define the generic form of a higher inductive type as follows.\todo{Use Agda syntax the same as in the rest of the paper}
\begin{center}
\begingroup
\fontsize{7.9pt}{0pt}\selectfont
\begin{align}
