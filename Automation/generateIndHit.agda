...
getPathClauseDep lpoints lpaths baseTyp baseRec (i ∷ is) (x ∷ xs) = bindTC (getPathClauseDep lpoints lpaths baseTyp baseRec is xs)
                     ...
                     (λ d → bindTC (debugPrint "tc.sample.debug" 12 (strErr "clause debugging --------> step 1" ∷ []))
                     (λ _ → bindTC (rmPars d y)
                     ...
                     (λ debug' → bindTC (debugPrint "tc.sample.debug" 12 (strErr "clause debugging -------->" ∷ []))
                     (λ _ → bindTC (debugPrint "tc.sample.debug" 12 (termErr debug' ∷ []))
                     (λ _ → returnTC ((clause (clvars ++L (vArg (con x laP) ∷ vArg (var "C") ∷ vars)) (def baseRec (clargs ++L (vArg (con x refargs) ∷ argTerms)))) ∷ xs'))))))))))))))))))))))))))
...
generateIndHit (arg i f) baseType indLs baseRec indType points paths =
  ...
  bindTC (debugPrint "tc.sample.debug" 20 (strErr "issue : generateInd1 -->" ∷ termErr funTypePath ∷ [])) λ _ → 
  bindTC (declareDef (arg i f) funTypePath) λ _ →
  bindTC (debugPrint "tc.sample.debug" 20 (strErr "issue : generateInd2 -->" ∷ termErr funTypePath ∷ [])) λ _ → 
  (defineFun f clause)
